...
const to = i => ({ x: 0, y: i * -4, scale: 1, display: 'auto', rot: -10 + Math.random() * 20, delay: i * 100 })
const from = i => ({ x: 0, rot: 0, scale: 1.5, display: 'auto', y: -1000 })
    ...
    return props.map(({ x, y, rot, scale, display }, i) => {
        return (
            <animated.div className="parent" key={i} style={{ transform: interpolate([x, y], (x, y) => `translate3d(${x}px,${y}px,0)`) }}>
                ...
                <animated.div className="child" {...bind(i)} style={{ display: display, transform: interpolate([rot, scale], trans), backgroundImage: `url(${cards[i]})` }} />
            </animated.div>
        );
    });
}
